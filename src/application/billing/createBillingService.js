...
function createBillingService(dependencies) {
  assertDependencies(dependencies, 'application.billing', { clock: ['now'], billingRepository: ['appendBillingEvent', 'upsertBillingUser', 'upsertBillingCustomer', 'getBillingCustomer', 'getBillingUser', 'listBillingUsers'], usersRepository: ['getUser', 'mergeUserFields'], stripeGateway: ['isConfigured', 'retrievePrice', 'listPromotionCodes', 'createCheckoutSession', 'retrieveCheckoutSession', 'updateSubscription', 'retrieveSubscription', 'createBillingPortalSession', 'constructWebhookEvent'] });
  const { clock, billingRepository, usersRepository, stripeGateway } = dependencies;
  ...
  try {
    ...
    if (String(userData.tierSource || '') !== 'manual') {
      ...
      await usersRepository.mergeUserFields(uid, {
        tier: userTier,
        tierSource: 'stripe',
        tierUpdatedAt: new Date(clock.now()),
      });
    }
  } catch (tierErr) {
...
async function getUserSubscriptionEntitlement(uid) {
  ...
  const promoUntil = d.promoUntil == null ? null : Number(d.promoUntil);
...
async function claimStripeCheckoutOwnership({ uid, customerId, subscriptionId, email, name }) {
  ...
  isStripeConfigured: stripeGateway.isConfigured,
  retrievePrice: stripeGateway.retrievePrice,
  ...
  updateSubscription: stripeGateway.updateSubscription,
  retrieveSubscription: stripeGateway.retrieveSubscription,
  createBillingPortalSession: stripeGateway.createBillingPortalSession,
  constructWebhookEvent: stripeGateway.constructWebhookEvent,
};
}
