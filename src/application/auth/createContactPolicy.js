...
function createContactPolicy(dependencies) {
  assertDependencies(dependencies, 'application.contactPolicy', { config: 'value', adminConfig: ['getContactPolicyConfig'], isCloudRuntime: 'value' });
  const { config, adminConfig, isCloudRuntime } = dependencies;
...
function canUseLocalDefaults() {
  return config.server.env !== 'production' && !isCloudRuntime;
}
...
module.exports = { createContactPolicy };
