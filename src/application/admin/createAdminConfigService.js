...
function createAdminConfigService(dependencies) {
  assertDependencies(dependencies, 'application.adminConfig', { adminConfigRepository: ['getTierConfigDoc', 'saveTierConfigDoc', 'getSeoConfigDoc', 'saveSeoConfigDoc', 'getAtlasConfigDoc', 'saveAtlasConfigDoc', 'getComponentRegistryDoc', 'saveComponentRegistryDoc', 'getContactPolicyDoc', 'saveContactPolicyDoc'], appConfig: 'value', setLangSmithRuntimeEnabled: 'function' });
  const { adminConfigRepository, appConfig, setLangSmithRuntimeEnabled } = dependencies;
...
const DEFAULT_ATLAS_CONFIG = {
  ...
  // ── 6. Model Routing ─────────────────────────────────────────────────────
  executionMode:            'multiagent',     // pure-model | single-agent | multiagent
  ...
  webSearchExecutionMode:   'agent',           // direct | agent
...
async function getAtlasConfig() {
  try {
    ...
    const cfg = {
      ...
      webSearchExecutionMode:   _str(d.webSearchExecutionMode,   D.webSearchExecutionMode),
...
async function upsertAtlasConfig(cfg) {
  ...
  const payload = {
    ...
    webSearchExecutionMode:   _str(cfg.webSearchExecutionMode,   D.webSearchExecutionMode),
...
module.exports = { createAdminConfigService };
