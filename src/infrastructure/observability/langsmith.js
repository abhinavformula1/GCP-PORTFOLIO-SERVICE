...
function createTracingAdapter({ config }) {
if (!config || !config.langsmith || !config.server) {
  throw new TypeError('tracingAdapter.config.langsmith and config.server are required');
}
...
function summarizeExecutionPlan(plan) {
  ...
  const timings = plan.supervisorNodeTimings && typeof plan.supervisorNodeTimings === 'object'
    ? plan.supervisorNodeTimings
    : {};
  return {
    ...
    webSearchExecutionMode: typeof plan.webSearchExecutionMode === 'string' ? plan.webSearchExecutionMode : '',
    webSearchExecution: typeof plan.webSearchExecution === 'string' ? plan.webSearchExecution : '',
    webSearchFallbackCode: typeof plan.webSearchFallbackCode === 'string' ? plan.webSearchFallbackCode : '',
    ...
    supervisorReason: previewText(plan.supervisorReason || '', 180),
    supervisorNodeTimings: Object.keys(timings).reduce(function (acc, key) {
      const value = Number(timings[key]);
      if (Number.isFinite(value)) acc[key] = value;
      return acc;
    }, {}),
    supervisorErrors: Array.isArray(plan.supervisorErrors) ? plan.supervisorErrors.slice(0, 8) : [],
  };
}
...
function summarizeWebSearchMeta(webSearch) {
  ...
  return {
    ...
    execution: webSearch.execution && typeof webSearch.execution === 'object'
      ? {
        mode: String(webSearch.execution.mode || webSearch.execution.execution || ''),
        fallbackCode: String(webSearch.execution.fallbackCode || ''),
        toolCalls: Number(webSearch.execution.toolCalls || 0),
        durationMs: Number(webSearch.execution.durationMs || 0),
      }
      : null,
  };
}
...
module.exports = { createTracingAdapter };
