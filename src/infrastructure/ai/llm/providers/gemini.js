...
function createGeminiProvider({ config, tracing }) {
if (!config || !config.gemini) {
  throw new TypeError('geminiProvider.config.gemini is required');
}
for (const method of [
  'traceIfEnabled', 'previewText', 'summarizeGenerationConfig',
  'summarizeUsage', 'summarizeStreamEvents',
]) {
  if (!tracing || typeof tracing[method] !== 'function') {
    throw new TypeError(`geminiProvider.tracing.${method} is required`);
  }
}
const {
  traceIfEnabled,
