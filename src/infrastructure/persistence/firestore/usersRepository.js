...
function createUsersRepository({ firestore }) {
if (!firestore || typeof firestore.getDb !== 'function') {
  throw new TypeError('usersRepository.firestore.getDb is required');
}
  ...
  return firestore.getDb().runTransaction(async (tx) => {
    ...
    if (snap.exists) {
      ...
      return {
        ...
        firstSeenAt: toMillis(existing.firstSeenAt),
        lastSeenAt: toMillis(existing.lastSeenAt),
