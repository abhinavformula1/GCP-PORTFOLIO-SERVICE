...
function createAnalyticsRepository({ firestore }) {
if (!firestore || typeof firestore.getDb !== 'function') {
  throw new TypeError('analyticsRepository.firestore.getDb is required');
}
...
async function fetchOverview(monthId, recentDayIds) {
  ...
  const documentData = (snapshot) => snapshot && snapshot.data ? (snapshot.data() || {}) : {};
  ...
  return {
    monthly,
    topPages: topPagesSnap && topPagesSnap.docs ? topPagesSnap.docs.map(documentData) : [],
    topReferrers: topRefsSnap && topRefsSnap.docs ? topRefsSnap.docs.map(documentData) : [],
    topCampaigns: topCampaignsSnap && topCampaignsSnap.docs ? topCampaignsSnap.docs.map(documentData) : [],
    recentUsers: recentUsersSnap && recentUsersSnap.docs ? recentUsersSnap.docs.map(userData) : [],
    dailySeries: dailySnaps.map((snapshot) => ({
      id: snapshot.id,
      data: snapshot.exists ? documentData(snapshot) : {},
    })),
  };
}
