...
function createAtlasRepository({ firestore, logger = console }) {
if (!firestore || typeof firestore.getDb !== 'function') {
  throw new TypeError('atlasRepository.firestore.getDb is required');
}
if (!logger || typeof logger.warn !== 'function') {
  throw new TypeError('atlasRepository.logger.warn is required');
}
...
function warnLocalFallback(scope, err) {
  ...
  logger.warn('[atlasRepository] using in-memory fallback for ' + key + ':', err.message);
}
...
module.exports = { createAtlasRepository };
