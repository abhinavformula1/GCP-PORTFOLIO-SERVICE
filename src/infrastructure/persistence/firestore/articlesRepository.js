...
function createArticlesRepository({ firestore }) {
if (!firestore || typeof firestore.getDb !== 'function') {
  throw new TypeError('articlesRepository.firestore.getDb is required');
}
...
function sanitiseArticleBlocks(blocks) {
  //console.log("### 7. Called from normaliseArticle :: ",blocks);
...
function normaliseArticle(id, data) {
  //console.log("### 5. Called from getArticle or listArticles :: ", id + ' ### 6. :: ' + data);
  const v = data || {};
...
async function getArticle(id) {
  console.log("### 3. Called from script :: ", id);
  const snap = await firestore.getDb().collection(SYSTEM_DESIGN_COLLECTION).doc(id).get();
 // console.log("### 4. Document retrived from firestore :: ", snap);
...
module.exports = { createArticlesRepository };