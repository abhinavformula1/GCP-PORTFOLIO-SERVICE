...
function createSalesforceTransport({ config, auditLogger = null }) {
if (!config || !config.salesforce) {
  throw new TypeError('salesforceTransport.config.salesforce is required');
}
if (auditLogger != null && typeof auditLogger !== 'function') {
  throw new TypeError('salesforceTransport.auditLogger must be a function');
}
      ...
      (res) => {
        ...
        res.on('end', () => {
          ...
          if (!skip && auditLogger) {
            ...
            Promise.resolve().then(() => auditLogger({
