...
async function getAnalyticsOverview({ month } = {}) {
  ...
  const recentUsers = overview.recentUsers.map((d) => {
    const data = d.data() || {};
    return {
      kind: data.kind || 'anon',
      name: data.name || null,
  ...
  const series = overview.dailySeries.map((snap) => {
