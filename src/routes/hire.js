...
router.post('/hire', hireLimiter, validateHire, async (req, res, next) => {
  ...
  try {
    ...
    if (sfConfigured) {
      // Correlation ID stamped on the Integration_Log__c row(s) generated by
      // this submission. Lets us trace request → Salesforce response →
      // resulting Recruiter_Inquiry__c via SOQL on Transaction_Id__c.
