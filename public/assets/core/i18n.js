...
const LANG = {
  en: {
    ...
    botConfirm: "Quick check — does this look right? Hit send and I'll get back to you over email.",
    botDone: function (name, email) { return 'Thank you so much, ' + name + " — your message means a lot. I'll review your details and email you at " + email + '. Looking forward to chatting!'; },
  ...
  fr: {
    ...
    botConfirm: 'Petite vérification — tout est correct? Appuyez sur Envoyer et je vous répondrai par email.',
    botDone: function (name, email) { return 'Merci beaucoup, ' + name + ' — votre message compte vraiment. Je vais examiner vos informations et vous écrire à ' + email + '. À très bientôt!'; },
...
export const PAGE_LANG = {
  en: {
    headerTitle: 'Senior Salesforce & GenAI Application Engineer',
    home: 'Home',
    ...
    systemDesignEyebrow: 'Software Architecture',
    ...
    welcomeSub:      'Senior Salesforce & GenAI Application Engineer',
    ...
    aboutMe: 'About Me',
    about1: 'Senior Application Engineer with 13+ years of experience building scalable enterprise systems across Salesforce and GCP, with hands-on production experience designing and deploying GenAI systems — including RAG pipelines, LLM evaluation frameworks, and AI observability tooling.',
    about2: 'Deep expertise in Salesforce (Apex, LWC, CPQ, Communications Cloud) combined with end-to-end GenAI engineering on Google Cloud — from chunking and vector search to retrieval evaluation and cost controls.',
    skills: 'Skills',
    ...
    proj0: ['Built an end-to-end RAG pipeline on GCP using Gemini 2.5 Flash, Firestore vector search, and a custom block-level chunking algorithm that preserves tables, code blocks, and document structure.', 'Implemented a retrieval evaluation framework measuring Recall@K, Precision@K, and MRR against a 50-question golden test set — achieving Recall@5 of 84% and MRR of 0.721, clearing the production gate.', 'Deployed AI observability admin tooling with a live SSE-streaming evaluation dashboard, per-question retrieval breakdowns, and pass/fail gates.', 'Operationalised with cost controls, model selection (Flash / Flash-Lite), RAG feature flagging, and automatic chunk indexing on article publish. Stack: Node.js, GCP Cloud Run, Firestore, Gemini API.'],
    ...
    proj4: ['Built an open-source platform that transforms Salesforce Apex debug logs into structured, token-efficient context for LLM-powered diagnostics.', 'Designed a deterministic parsing and context extraction pipeline that reconstructs execution flow, exceptions, SOQL, DML operations, and governor limit insights.', 'Integrated the platform with Cursor through the Model Context Protocol (MCP), enabling developers to analyze logs directly from AI chat.', 'Generates structured Markdown optimized for AI reasoning, reducing unnecessary prompt tokens by more than 80% versus raw log input.'],
  },
  fr: {
    headerTitle: 'Ingénieur Senior Salesforce & GenAI',
    home: 'Accueil',
    ...
    systemDesignEyebrow: 'Architecture logicielle',
    ...
    aboutMe: 'À Propos',
    about1: 'Ingénieur Senior en Applications Salesforce avec plus de 13 ans d\'expérience en développement, architecture et DevOps Salesforce, spécialisé dans la création d\'applications d\'entreprise évolutives avec Apex, Lightning Web Components et des intégrations API.',
    ...
    jobTenureCurrent: '2+ ans',
    jobTenureFive: '5+ ans',
    jobTenureOne: '1 an',
    jobTenureThree: '3 ans',
    viewEarlierExperience: 'Voir les expériences précédentes',
    ...
    proj0: ["Développement d'un pipeline RAG complet sur GCP avec Gemini 2.5 Flash, Firestore Vector Search et un algorithme de découpage personnalisé préservant les tableaux et blocs de code.", "Implémentation d'un framework d'évaluation mesurant Recall@K, Precision@K et MRR sur un jeu de test doré de 50 questions — Recall@5 de 84% et MRR de 0,721.", "Déploiement d'outils d'observabilité IA avec tableau de bord SSE en temps réel, résultats par question et seuils de validation production.", "Opérationnalisation avec contrôle des coûts, sélection de modèle, feature flag RAG et indexation automatique. Stack : Node.js, GCP Cloud Run, Firestore, API Gemini."],
    ...
    proj4: ["Création d'une plateforme open source qui transforme les debug logs Apex Salesforce en contexte structuré et économe en tokens pour des diagnostics pilotés par LLM.", "Conception d'un pipeline déterministe d'analyse et d'extraction de contexte reconstruisant le flux d'exécution, les exceptions, SOQL, DML et les insights sur les governor limits.", "Intégration de la plateforme avec Cursor via le Model Context Protocol (MCP), permettant aux développeurs d'analyser les logs directement depuis le chat IA.", "Génère un Markdown structuré optimisé pour le raisonnement IA, réduisant de plus de 80% les tokens inutiles par rapport à une entrée brute des logs."],
  },
};
